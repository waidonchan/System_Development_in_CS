function onFormSubmit(e) {
    ...
    const newDoc = templateDoc.makeCopy('棚報告書(団体用) - ' + club_name);
    ...
    const pdfFileName = '棚利用報告書 - ' + club_name + '.pdf';
    const pdfFile = newFolderDestination.createFile(pdf);
    pdfFile.setName(pdfFileName);
  
    // メール本文を生成
    const subject = '売店横の棚の報告書の提出： ' + club_name;
    const emailBody = `【学校の担当の課の名前】
    ご担当者様 (cc: ${club_name} ${representative_name}さん)
  
    お世話になっております。福井県立大学 CSサークルです。
    
    先日、売店横の棚で${purpose}を行っていた、${club_name}の報告書を提出いたします。
  
    ご不明点がございましたら、このメールへの返信にてお知らせください。CSサークルおよび代表である${club_name}の代表である${representative_name}さんが対応させていただきます。
