function onFormSubmit(e) {
    ...
    var purpose = values[10]; // 目的を取得
    var result = values[11]; // 結果の情報を取得
    var memo = values[12]; // 備考を取得
  
    // テンプレートドキュメントのIDを指定
    const templateDocId = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'; // ★テンプレートドキュメントのIDを指定
    const templateDoc = DriveApp.getFileById(templateDocId);
   
    // テンプレートドキュメントをコピーして新しいドキュメントを作成
    const newDoc = templateDoc.makeCopy('チャレンジスペース報告書(団体用) - ' + club_name);
    ...
    // メールを送信
    MailApp.sendEmail({
      to: 'school@example', // 就職・生活支援課のメールアドレスに変更
      cc: contact_mail,
      subject: subject,
      body: emailBody,
    ...
    // フォルダ情報をログに出力（必要であれば削除できます）
    Logger.log('Folder ID: ' + folder_information);
  }
