function postToSlack() {
    ...
    data.forEach(function (row) {
      ...
      if (areDatesEqual(oneDaysBefore, today)) {
        var message = "リマインド：" + clubName + "のチャレンジスペースの利用予定日まであと一日です！！日程を再確認しましょう！\n詳細はこちら：" + doc;
        messages.push(message);
        sendReminderEmail(email, "リマインド：学生会館 チャレンジスペースの利用予定日（1日前）", message, doc, representativeName, clubName);
      }
  
      // 一週間前の通知
      if (areDatesEqual(oneWeekBefore, today)) {
        var message = "リマインド：" + clubName + "のチャレンジスペースの利用予定日まであと一週間です！日程を確認しましょう！\n詳細はこちら：" + doc;
        messages.push(message);
        sendReminderEmail(email, "リマインド：学生会館 チャレンジスペースの利用予定日（一週間前）", message, doc, representativeName, clubName);
      }
    });
  ...
  function postMessageToSlack(message) {
    var slackWebhookUrl = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"; // ★SlackのWebhook URLを設定
  ...
  ${clubName}の学生会館チャレンジスペースの利用予定日が近づいております。あと少しで当日となりますね。 準備や確認事項がありましたら、ぜひこの機会にご確認ください。
