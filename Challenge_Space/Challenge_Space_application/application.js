function onFormSubmit(e) {
    ...
    var information = values[13]; // 利用の概要を取得
    var sale_image = values[14]; // 販売物の写真1を取得
    var sale_image2 = values[15]; // 販売物の写真2を取得
    var sale_image3 = values[16]; // 販売物の写真3を取得
    var memo = values[17]; // 備考を取得
  
    // 写真を挿入
    if (sale_image) {
      sale_image = sale_image.replace('https://drive.google.com/open?id=', '');
      var attachImg = DriveApp.getFileById(sale_image).getBlob();
    }
    ...
    if (start_time === '' || end_time === '') {
      ...
      event = calendar.createAllDayEvent('チャレンジスペース利用 - ' + club_name, startDate, endDate, {description: information});
    } else {
      event = calendar.createEvent('チャレンジスペース - ' + club_name, startDate, endDate, {description: information});
    }
    ...
    var folder_information = club_name + ' ' + start_date;
    ...
    // 親フォルダのIDを指定
    const parentFolderId = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxx'; // ★親フォルダのIDを記載
    ...
    // テンプレートドキュメントのIDを指定
    const templateDocId = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'; // ★テンプレートドキュメントのIDを記載
    const templateDoc = DriveApp.getFileById(templateDocId);
    const newDoc = templateDoc.makeCopy('チャレンジスペース利用申込書(団体用) - ' + club_name, newFolder);
    ...
    // スプレッドシートのID
    const spreadsheetId = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'; //★スプレッドシートのIDを記載
    ...
    MailApp.sendEmail({
      to: 'school@example', // ★学校の担当の課のメールアドレスを記載
      cc: contact_mail,
      subject: subject,
      body: emailBody,
