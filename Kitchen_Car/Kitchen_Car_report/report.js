function onFormSubmit(e) {
    ...
    // テンプレートドキュメントのIDを指定
    const templateDocId = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'; // ★テンプレートドキュメントのIDに変更
    const templateDoc = DriveApp.getFileById(templateDocId);
   
    // テンプレートドキュメントをコピーして新しいドキュメントを作成
    const newDoc = templateDoc.makeCopy('キッチンカー報告書(団体用) - ' + club_name);
    ...
    // 親フォルダのIDを指定
    const parentFolderId = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'; // ★親フォルダのメールアドレスに変更
    ...
    // メールを送信
    MailApp.sendEmail({
      to: 'school@example', // ★学校の担当の課のメールアドレスに変更
      cc: contact_mail,
      subject: subject,
      body: emailBody,
